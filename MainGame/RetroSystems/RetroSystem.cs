...
public abstract class RetroSystem
{
		...
		int				firstGid = 1 )
	{
		...
		for( int i = 0; i < count; i++ )
		{
			...
			if( GlobalTileColorIndex >= 0 && MaxLocalTileColors > 0 )
			{
				...
				allowedLocalTile = [.. locals.Take(MaxLocalTileColors)];
			}
			...
			for( int ty = 0; ty < target; ty++ )
			{
				...
				for( int tx = 0; tx < target; tx++ )
				{
					...
					if( DoubleWidePixels )
					{
						srcCol = Math.Min(srcCol & ~1, nativeW - 1);
					}
	...
	public AnimatedSprite BuildCharacterSprite( GraphicsDevice gd, CharacterAppearance a )
	{
		...
		var sprite = new AnimatedSprite( texture, CharWidth, CharHeight, new Vector2( CharTileWidth, CharTileHeight ) );
		...
		Func<int, Color>	resolve)
	{
		...
		if( MaxSpriteSemanticColors > 0 )
		{
			...
			{
				allowedSemantic = [.. semantics.Take( MaxSpriteSemanticColors )];
			}
		}
