...
public class NESPalette : Palette
{
	...
	public NESPalette() : base(
		[
			DarkBackground, WarmAmber, MediumBrown, DarkBrown, VividRed, DarkRed,
			PaleYellow, YellowGreen, NearWhite, WallGray, LightWall, DoorBlack,
			WarmDoorWood, NesBlue, DarkNesBlue, Aqua, NesGreen, DarkGreen,
			RoadBlack, RoadGray, SidewalkGray, SidewalkLight, RustOrange, ShelfTan,
			LeafGreen, DarkLeaf, CounterTan, CounterDark, DarkestBrown, SkinDark,
			SkinMedium, SkinTan, PaleCyan, MidMagenta, SkinLight, SkinFair,
			SkinPale, BrightYellow,
		])
	{
	}
}
